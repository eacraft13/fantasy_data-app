$(function () {
    ...
    .success(function(lastCompletedWeek) {
        ...
        $('p.lead').html('Defense Game Stats - Week ' + lastCompletedWeek);

        async.parallel(
            _.map(_.range(lastCompletedWeek), function(week) {
                return function(callback) {
                    $.ajax({ url: '/api/nfl/v2/JSON/FantasyDefenseByGame/2015/' + (week + 1) })
            ...
            function(err, results) {
                ...
                var columns = _.map(_.keys(data[0]), function(val, key) {
                    return {
                        field: val,
                        title: val.replace(/([a-z](?=[A-Z\d]))/g, '$1 ').replace(/\d(?=[A-Za-z])/g, '$1 '),
                        sortable: true,
